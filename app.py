...
from aluno_banco import AlunoBanco


class App:
    def __init__(self):
        self.db = AlunoBanco('localhost', 'root', 'escola', '') # Objeto de acesso a dados / DAO = DATA ACCESS OBJECT
        ...
        self.cursos = ['Python', 'Javascript', 'Django', 'React Js']
    ...
    def atualizarTabela(self):
        ...
        for aluno in alunos:
            self.tabela.insert('', END, values = (aluno['matricula'], # Dados vêm como dicionário, não como objeto
                                                  aluno['nome'],
                                                  aluno['idade'],
                                                  aluno['curso'],
                                                  aluno['nota']))
    ...
    # Melhoria para o código ("criar aluno" se repete nas funções add e editar)
    def criarAluno(self):
    ...
    def deletarAluno(self):
        ...
        messagebox.askyesno('Tem certeza?', 'Deseja excluir os dados do aluno?')
        self.limparCampos()
        self.atualizarTabela()
